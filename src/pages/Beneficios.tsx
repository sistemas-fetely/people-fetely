...
import {
  useBeneficios, useCriarBeneficio, useEditarBeneficio, useExcluirBeneficio,
  type BeneficioComColaborador,
...
export default function Beneficios() {
  ...
  const criarMut = useCriarBeneficio();
  const editarMut = useEditarBeneficio();
  const excluirMut = useExcluirBeneficio();

  const [busca, setBusca] = useState("");
  const [filtroTipo, setFiltroTipo] = useState("todos");
  const [showForm, setShowForm] = useState(false);
  const [editing, setEditing] = useState<BeneficioComColaborador | null>(null);

  // Form state
  const [formColaboradorId, setFormColaboradorId] = useState("");
  const [formTipo, setFormTipo] = useState("vt");
  ...
  const { data: colaboradores = [] } = useQuery({
    queryKey: ["colaboradores_select_beneficios"],
    queryFn: async () => {
  ...
  const filtered = useMemo(() => {
    return beneficios.filter((b) => {
      const matchBusca =
        (b.colaborador?.nome_completo ?? "").toLowerCase().includes(busca.toLowerCase()) ||
        (b.operadora ?? "").toLowerCase().includes(busca.toLowerCase()) ||
        (b.colaborador?.departamento ?? "").toLowerCase().includes(busca.toLowerCase());
  ...
  const custoEmpresa = beneficios
    .filter((b) => b.status === "ativo")
    .reduce((s, b) => s + Number(b.valor_empresa), 0);
  const descontoTotal = beneficios
    .filter((b) => b.status === "ativo")
    .reduce((s, b) => s + Number(b.valor_desconto), 0);
  const colaboradoresComBeneficio = new Set(
    beneficios.filter((b) => b.status === "ativo").map((b) => b.colaborador_id)
  ).size;
  ...
  const openEdit = (b: BeneficioComColaborador) => {
    setEditing(b);
    setFormColaboradorId(b.colaborador_id);
    setFormTipo(b.tipo);
    setFormDescricao(b.descricao || "");
    setFormOperadora(b.operadora || "");
    setFormNumeroCartao(b.numero_cartao || "");
    setFormValorEmpresa(Number(b.valor_empresa));
    setFormValorDesconto(Number(b.valor_desconto));
    setFormDataInicio(b.data_inicio);
    setFormDataFim(b.data_fim || "");
    setFormStatus(b.status);
    setFormObs(b.observacoes || "");
    setShowForm(true);
  };
  ...
  const handleSubmit = () => {
    if (editing) {
      editarMut.mutate({
        ...
        operadora: formOperadora || null,
        numero_cartao: formNumeroCartao || null,
        valor_empresa: formValorEmpresa,
        valor_desconto: formValorDesconto,
      ...
      if (!formColaboradorId) return;
      criarMut.mutate({
        colaborador_id: formColaboradorId,
        tipo: formTipo,
        descricao: formDescricao || undefined,
        operadora: formOperadora || undefined,
        numero_cartao: formNumeroCartao || undefined,
        valor_empresa: formValorEmpresa,
        valor_desconto: formValorDesconto,
        data_inicio: formDataInicio,
          ...
          <TableBody>
            {filtered.length === 0 ? (
              ...
              filtered.map((b) => {
                ...
                return (
                  <TableRow key={b.id}>
                    <TableCell className="font-medium">{b.colaborador?.nome_completo}</TableCell>
                    <TableCell>{b.colaborador?.departamento}</TableCell>
                    <TableCell>
                    ...
                    {canManage && (
                      <TableCell className="text-right space-x-1">
                        ...
                        {isAdmin && (
                          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => excluirMut.mutate(b.id)}>
      ...
      <Dialog open={showForm} onOpenChange={(o) => { if (!o) { setShowForm(false); resetForm(); } else setShowForm(true); }}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSubmit} disabled={!editing && !formColaboradorId}>
